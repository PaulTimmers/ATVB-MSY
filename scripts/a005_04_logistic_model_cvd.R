...
for (trait in c("fath_heart_disease", "moth_heart_disease", "bro_heart_disease")) {
  ph[,trait] <- ifelse(ph[,trait] > 1, TRUE, FALSE)
}

# Subset data frame
y <- c("cvd", "fath_heart_disease", "moth_heart_disease") # Dependent variables
...
fwrite(ph1, "st005_04_cvd_geo_vars.tsv", sep="\t", quote=TRUE, na="NA") # Write to file for later retrieval
...
for(haplo in cvd_groups) {
  writeLines(haplo)
  fs <- paste0("cvd ~ `",haplo,"` + array + age + I(age^2) + ",paste0(geovars,collapse=" + ")," + ",paste0("pc",1:n_pcs, collapse=" + "))
  ms <- speedglm(as.formula(fs), family=binomial(link="logit"), data=ph1, model=TRUE)
  ss <- summary(ms)
  son_coef <- ss$coefficients[grepl(paste0("^`",haplo),rownames(ss$coefficients)),,drop=FALSE]
  names(son_coef) <- c("beta","se","z","p")
...
for(haplo in sig_haplos) {
  writeLines(haplo)
  ff <- paste0("fath_heart_disease ~ `",haplo,"` + array + fath_age + I(fath_age^2) + ",paste0(geovars,collapse=" + ")," + ",paste0("pc",1:n_pcs, collapse=" + "))
  mf <- speedglm(as.formula(ff), family=binomial(link="logit"), data=ph1, model=TRUE)
  sf <- summary(mf)
  fath_coef <- sf$coefficients[grepl(paste0("^`",haplo),rownames(sf$coefficients)),,drop=FALSE]
  names(fath_coef) <- c("beta","se","z","p")

  fm <- paste0("moth_heart_disease ~ `",haplo,"` + array + moth_age + I(moth_age^2) + ",paste0(geovars,collapse=" + ")," + ",paste0("pc",1:n_pcs, collapse=" + "))
  mm <- speedglm(as.formula(fm), family=binomial(link="logit"), data=ph1, model=TRUE)
  sm <- summary(mm)
  moth_coef <- sm$coefficients[grepl(paste0("^`",haplo),rownames(sm$coefficients)),,drop=FALSE]
  names(moth_coef) <- c("beta","se","z","p")
...
fwrite(res, "st005_04_cvd_geo_model.csv", sep=",", na="NA", quote=FALSE)
...
ggsave("st005_04_cvd_geo_model_i1.pdf", p1, width=6, height=2)
...
ggsave("st005_04_cvd_geo_model.pdf", p1, width=6, height=3)
